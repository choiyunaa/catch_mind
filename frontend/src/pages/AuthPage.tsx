  ...
  return (
    <div style={{ maxWidth: 400, margin: '60px auto', padding: 24, border: '1px solid #eee', borderRadius: 8 }}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button type="submit" style={{ width: '100%', padding: 12 }} disabled={loading}>
          {loading ? '처리 중...' : mode === 'login' ? '로그인' : '회원가입'}
        </button>
      </form>
      <div style={{ marginTop: 16, textAlign: 'center' }}>
        <button onClick={() => { setMode(mode === 'login' ? 'register' : 'login'); reset(); }} style={{ background: 'none', border: 'none', color: '#007bff', cursor: 'pointer' }}>
          {mode === 'login' ? '회원가입' : '로그인'}으로 전환
        </button>
      </div>
      {message && <div style={{ marginTop: 16, color: message.includes('성공') ? 'green' : 'red', textAlign: 'center' }}>{message}</div>}
    </div>
  );
};
